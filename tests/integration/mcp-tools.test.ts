...
const validCalls: Array<{ name: string; args: Record<string, unknown> }> = [
  { name: "dyslexai_profile_get", args: {} },
  { name: "dyslexai_profile_update", args: { activeProfile: "balanced" } },
  {
    name: "dyslexai_rewrite",
    args: {
      text: "fo modif le src/tools/handlers.ts psk dyslexai_rewrite marche pa avec --preserveTechnicalTokens et @modelcontextprotocol/sdk",
      mode: "readable",
      preserveTechnicalTokens: true,
      showChanges: true
    }
  },
  { name: "dyslexai_explain", args: { kind: "error", text: "TS2345 src/tools/handlers.ts:42 callDyslexAITool invalid argument" } },
  { name: "dyslexai_structure_instructions", args: { text: "Create tests. Do not publish. Verify package." } },
  { name: "dyslexai_summarize_terminal", args: { text: "ok\nwarn deprecated\nError: root failure\nexit code 1" } },
  { name: "dyslexai_explain_diff", args: { text: "--- a/file.ts\n+++ b/file.ts\n-old\n+new" } },
  { name: "dyslexai_check_ambiguity", args: { text: "Use 07/08/2026, 1.500, and 10m." } },
  { name: "dyslexai_compare_versions", args: { original: "hello teh ./src/a.ts", revised: "hello the ./src/a.ts" } },
  { name: "dyslexai_resume_context", args: { text: "Decided: use stdio.\nDone: build passed.\nRemaining: tarball test." } },
  { name: "dyslexai_doctor", args: {} }
];
...
test("all MCP tools accept valid calls and preserve structured results", async () => {
  ...
  try {
    ...
    const rewrite = await client.callTool({
      name: "dyslexai_rewrite",
      arguments: validCalls.find((call) => call.name === "dyslexai_rewrite")?.args
    });
    const text = JSON.stringify(rewrite.structuredContent);
    assert.match(text, /src\/tools\/handlers\.ts/);
    assert.match(text, /dyslexai_rewrite/);
...
test("all MCP tools return user errors for invalid calls and server remains functional", async () => {
  ...
  try {
    for (const { name } of validCalls) {
      ...
      const after = await client.callTool({ name: "dyslexai_profile_get", arguments: {} });
